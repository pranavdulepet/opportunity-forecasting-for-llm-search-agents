"""Label Paper Search decision states with Monte Carlo continuations."""
...
from opportunity_forecasting.data.environments import ENV_DOMAIN_CHOICES, build_env_from_args, reward_mode_choices
...
def _require_vllm() -> None:
    if LLM is None or SamplingParams is None:
        raise RuntimeError(
            ...
            "Create the full environment with `conda env create -f environments/training.yml`. "
...
def _best_reward_from_seen_products(
    ...
    for _, info in (seen_products or {}).items():
        if not isinstance(info, dict):
            continue
        for k in (COUNTERFACTUAL_REWARD_KEY, "paper_relevance_reward", "reward"):
            v = info.get(k, None)
            if v is None:
                continue
            try:
...
def replay_to_checkpoint(
    ...
    for action in prefix_actions:
        ...
        try:
            current_asins = extract_asins_from_observation(obs, env=env)
...
def _prefix_is_terminal(prefix_actions: Sequence[str]) -> bool:
    if not prefix_actions:
        return False
    last_action = str(prefix_actions[-1] or "").strip().lower()
    return last_action.startswith("stop") or "buy now" in last_action
...
def _build_action_prompt(
    ...
    if hist_k == 60 and obs_char_limit == 1200 and action_display_limit is None and not allow_numeric_answer:
        return SELECTION_PROMPT_QWEN.format(
            goal=goal,
    ...
    return (
        "You are a search assistant. Select the single best action from the list to progress toward the goal.\n\n"
        f"Search Goal: {goal}\n\n"
...
def _vllm_generate_texts(
    ...
    try:
        req_outs = llm.generate(prompts, sampling_params_list, use_tqdm=False)
        texts: List[str] = []
        for o in req_outs:
            if not o.outputs:
                texts.append("")
...
def _final_reward_from_env(env) -> float:
    if hasattr(env, "get_final_reward"):
        try:
            return float(env.get_final_reward())
        except Exception:
            pass
...
def _current_best_for_continuation(
    *,
    env,
    obs: str,
    seen_products: Dict[str, dict],
    ...
    """Return the best candidate utility known at the current continuation state."""
    try:
        update_seen_products(obs, seen_products, env)
    except Exception:
        pass
    ...
    try:
        env_final = float(_final_reward_from_env(env))
        if env_final >= 0.0:
            best_val = max(best_val, env_final)
    except Exception:
        pass
...
def run_continuations_lockstep(
    ...
    *,
    ...
    final_best: List[Optional[float]] = [None for _ in range(K)]


    for env in envs:
        obs, replayed_best, replayed_seen = replay_to_checkpoint(
            env=env,
            goal_idx=goal_idx,
            prefix_actions=prefix_actions,
            reward_mode=reward_mode,
        )
        replayed_seen = _sanitize_seen_products_for_reward_mode(
            replayed_seen,
            reward_mode=reward_mode,
        )
        replayed_best = max(
            float(replayed_best),
            _best_reward_from_seen_products(replayed_seen, reward_mode=reward_mode),
        )
        obses.append(obs)
        histories.append(list(prefix_actions))
        seen_products_list.append(replayed_seen)
        best_seen.append(float(replayed_best))
    ...
    for step in range(int(max_steps)):
        ...
        for j, va in enumerate(active_valid_actions):
            if not va:
                idx = active_idxs[j]
                done_flags[idx] = True
                final_best[idx] = _current_best_for_continuation(
                    env=envs[idx],
                    obs=obses[idx],
                    seen_products=seen_products_list[idx],
                    best_seen=best_seen[idx],
                    reward_mode=reward_mode,
                )
    ...
    for i in range(K):
        if final_best[i] is None:
            final_best[i] = _current_best_for_continuation(
                env=envs[i],
                obs=obses[i],
                seen_products=seen_products_list[i],
                best_seen=best_seen[i],
                reward_mode=reward_mode,
            )

    return [float(max(0.0, value if value is not None else 0.0)) for value in final_best]


def paper_search_fields(checkpoint: dict[str, Any]) -> dict[str, Any]:
    return {
        "visited_product_page": bool(checkpoint.get("visited_product_page", False)),
        "has_opened_paper": bool(
            checkpoint.get(
                "has_opened_paper",
                checkpoint.get("visited_product_page", False),
            )
        ),
        "current_paper_id": str(checkpoint.get("current_paper_id", "") or ""),
        "visible_result_ids": list(checkpoint.get("visible_result_ids", []) or []),
        "opened_paper_ids": list(checkpoint.get("opened_paper_ids", []) or []),
        "num_opened_papers": int(checkpoint.get("num_opened_papers", 0) or 0),
        "num_seen_papers": int(checkpoint.get("num_seen_papers", 0) or 0),
    }


def label_checkpoint_vllm(
    env_for_baseline: Any,
    envs_for_continuations: Optional[List[Any]],
    llm: "LLM",
    tokenizer: Optional[Any],
    ...
    *,
    ...
    terminal_prefix = _prefix_is_terminal(prefix_actions)
    if terminal_prefix:
    ...
    if terminal_prefix or effective_max_steps <= 0:
        max_future_bests = [float(best_reward_seen) for _ in range(int(num_continuations))]
    elif batch_continuations:
    ...
    is_paper_search = getattr(env_for_baseline, "ccs_domain", "") == "paper_search"
    paper_fields = paper_search_fields(checkpoint) if is_paper_search else {}
    ...
    return {
        ...
        "metadata": {
            ...
            **({"terminal_prefix": bool(terminal_prefix)} if is_paper_search else {}),
        },
    }


def build_batched_continuation_envs(args: argparse.Namespace, baseline_env: Any) -> List[Any]:
    """Build K independent continuation envs without reloading static WebShop data.

    WebShop's simulator stores the full product catalog, search index, and goal
    list on the server object. For Monte Carlo continuations those structures
    are immutable; only session/browser state must be independent. Sharing the
    server preserves semantics while avoiding K redundant catalog loads per job.
    """

    k = int(args.num_continuations)
    domain = str(getattr(args, "domain", "webshop"))
    if domain == "webshop" and hasattr(baseline_env, "server"):
        env_cls = type(baseline_env)
        observation_mode = getattr(baseline_env, "observation_mode", "text")
        return [
            env_cls(
                observation_mode=observation_mode,
                server=baseline_env.server,
                session_prefix=f"mc{k_idx}_",
            )
            for k_idx in range(k)
        ]
    return [build_env_from_args(args) for _ in range(k)]
...
def main():
    ap = argparse.ArgumentParser(description="Label checkpoints with Monte Carlo continuations (vLLM backend)")
    ...
    ap.add_argument("--paper_query_path", type=str, default="")
    ap.add_argument("--paper_corpus_path", type=str, default="")
    ap.add_argument("--paper_qrels_path", type=str, default="")
    ap.add_argument("--paper_dataset_name", type=str, default="princeton-nlp/LitSearch")
    ap.add_argument("--paper_query_config", type=str, default="query")
    ap.add_argument("--paper_corpus_config", type=str, default="corpus_clean")
    ap.add_argument("--paper_split", type=str, default="full")
    ap.add_argument("--paper_page_size", type=int, default=10)
    ap.add_argument("--paper_max_results", type=int, default=50)
    ap.add_argument(
        "--reward_mode",
        type=str,
    ...
    ap.add_argument("--gpu_memory_utilization", type=float, default=0.88, help="vLLM GPU memory utilization target (0-1).")
    ap.add_argument("--max_model_len", type=int, default=4096, help="vLLM max model length override.")
    ap.add_argument(
        "--dtype",
        type=str,
        default="half",
    ...
    print(f"Initializing {args.domain} environment(s)...", flush=True)
    env_baseline = build_env_from_args(args)
    cont_envs: Optional[List[Any]] = None
    if args.batch_continuations:
        cont_envs = build_batched_continuation_envs(args, env_baseline)
    ...
    with output_path.open(mode) as f:
        for i, ckpt in enumerate(checkpoints):
            ...
            if (i + 1) % progress_every == 0 or (i + 1) == len(checkpoints):
    ...
    if hasattr(env_baseline, "close"):
        env_baseline.close()
    for env in cont_envs or []:
